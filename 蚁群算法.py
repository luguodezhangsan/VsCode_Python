...
class Ant(object):
    ...
    def __clean_data(self):
        ...
        self.open_table_city[city_index] = False  # 遍历
        self.move_count = 1
    ...
    def __choice_next_city(self):
        ...
        next_city = -1
        select_city_prob = [0.0 for i in range(city_num)]  # 存储去下一个城市的概率
        total_prob = 0.0
        ...
        for i in range(city_num):
            if self.open_table_city[i]:
                try:
                    ...
                    print('Ant ID:{ID}, current city:{current}, target city:{target}'.format(ID = self.ID, current = self.current_city, target = next_city))
                    sys.exit(1)
...
class TSP(object):
    ...
    def new(self, evt = None):
        ...
        for i in range(len(distance_x)):
            ...
            # 显示坐标
            self.canvas.create_text(10*x, 5*y-10,
                    text = '(' +str(x)+','+str(y)+')',
                    fill = 'black'
                    )
        ...
        self.best_ant.total_distance = 1 << 31  # 初始化最大距离
        self.iter = 1  # 初始化迭代次数
    ...
    def __update_pheromone_gragh(self):
        ...
        for ant in self.ants:
            for i in range(1, city_num):
                ...
                # 在路径上的每两个相邻城市留下的信息素，与路径总距离成反比
                temp_pheromone[start][end] += Q / ant.total_distance
                temp_pheromone[start][end] = temp_pheromone[start][end]

        # 更新所有城市之间的信息素，旧信息素衰减加上新的迭代信息素
        for i in range(city_num):
            for j in range(city_num):
